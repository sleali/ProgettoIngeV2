...
public class N 
{
	...
	{
		boolean exist = alreadyExist(e);  //se l'elemento esiste già avrò exist = true
		if(!exist) //posso procedere all'aggiunta solo se exist == false
			rete.add(e);
		return !exist; //se l'aggiunta e' andata a buon fine ritorno true, ma per far ciò significa che exist == false
	}
		...
		try 
		{
			...
			source = new BufferedReader(new FileReader(f));
			 do 
			 {
				 ...
				 if (! (s == null))
				 {
					 e = gson.fromJson(s, Elemento.class);
					 this.rete.add(e);
				 }
			 } 
			 while (! (s == null));
	...
	public boolean isEqual(N retePar)
	{
